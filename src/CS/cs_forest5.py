...
skip_cols = 2
#Wczytanie danych
dataset = pd.read_csv('text_cs',header=0, delimiter=" ")#, usecols=range(skip_cols,16))
...
Y_names = names[-1]
#dataset[Y_names] = dataset[Y_names].astype(int)
...
cs = RandomForestClassifier(max_depth=5, n_estimators=100)
...
plt.scatter(X_test['LP'], Y_pred, color='blue', marker='x')
