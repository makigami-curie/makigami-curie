#UNIVARIATE FEATURE SELECTION - F-SCORE
...
for i in range(1,len(X_names)+1):
	regFS = SelectKBest(f_regression, i)
	regFS.fit(X_train, Y_train)
	X_tr = regFS.transform(X_train)
	reg.fit(X_tr, Y_train)

	X_tr = regFS.transform(X_test)
#print(regFS.support_)
#print(regFS.ranking_)
