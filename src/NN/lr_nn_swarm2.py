...
def f_one(x):
	N = math.ceil(x[0])
	hid = list(int(y) for y in x[1:N+1] )
	reg = MLPRegressor(hidden_layer_sizes=(hid) )#,tol=1e-6,max_iter=1000, verbose=True)#, len(X_names)-2))
...
opt = ps.GlobalBestPSO(n_particles=20, dimensions=dim, options=options, bounds=bons)
...
N = math.ceil(pos[0])
hid = list(int(y) for y in pos[1:N+1] )
reg = MLPRegressor(hidden_layer_sizes=(hid) )
reg.fit(X_train, Y_train)
Y_pred = reg.predict(X_test)
