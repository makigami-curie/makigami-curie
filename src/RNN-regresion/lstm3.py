...
#		self.lstm3 = nn.LSTM(n_hidden1, n_output)
		self.lin1 = nn.Linear(n_hidden1, n_hidden1)
		self.lin2 = nn.Linear(n_hidden1, n_output)
		self.p = 0.5

	def forward(self, seq, hc = None):
		out = []
		if hc == None:
			hc1 = None#, hc2, hc3 = None, None, None
		else:
			nc1 =hc #, hc2, hc3 = hc
...
#			x_in, hc3 = self.lstm3(tmp, hc3)
			tmp = F.relu(self.lin1(tmp))
			x_in = self.lin2(tmp)
			out.append(x_in)
		return torch.stack(out).squeeze(1), hc1#(hc1, hc2, hc3)
