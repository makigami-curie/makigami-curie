...
def clean_data(dataset, Y_names):
...
#wybieranie tylko istotnych pol - takich ktorych wartosci nie zmieniaja sie w 'kazdym' rekordzie
	N = len(dataset)
	CUTOFF = 0.2
	char_columns = dataset.dtypes.pipe(lambda x: x[x=='object']).index
	for col in char_columns:
#		print(col, len(dataset[col].unique()) , CUTOFF * N, len(dataset[col].unique()) > CUTOFF * N and col != Y_names)
#		print(col, Y_names, col != Y_names)
		if len(dataset[col].unique()) > CUTOFF * N and col != Y_names: skip_columns.append(dataset.columns.get_loc(col))
#usuniecie ewentualnych powtorzen
...
#	Y_names = 'TIME'
	...
	for i, col in enumerate(namesS):
		...
		if (dataset[col].dtype != 'object') and (dataset[col].dtype !='bool') and (col != 'WORKER') and (col != Y_names):
			dataset[col] = sc[col].fit_transform(pd.DataFrame(dataset[col]))
#	print(i,col)
#		dataset[col] = scale(dataset[col])

	dummis_columns = np.setdiff1d(dummis_columns, ['WORKER', Y_names])
